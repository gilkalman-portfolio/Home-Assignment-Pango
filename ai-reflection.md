...
The second notable finding is BUG-02: the word "error" appears in a billing confirmation message. Regardless of the technical cause, this is what the user sees after paying — in any payment system, that is unacceptable. Combined with BUG-11 (user deletion returning a false server error) and BUG-10 (history bypassing validation), this suggests server-side logic is unreliable and operates independently of what the UI expects.
